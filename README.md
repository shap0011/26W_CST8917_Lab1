...
## Part 12 - Create the Database in Azure

An Azure Cosmos DB account using the Core (SQL) API and serverless capacity mode was created in the `rg-serverless-lab1` resource group.  
A database named `textanalyzer-db` and a container named `analysis-results` were created to store JSON analysis results generated by the TextAnalyzer function.

*Figure 17: Azure Cosmos DB (serverless) account created in the same resource group as the Function App.*\
![Cosmos DB account created](./screenshots/17-cosmosdb-created.png)

*Figure 18: Database and container created for storing Text Analyzer results.*\
![Database and container created](./screenshots/18-cosmosdb-database-container.png)
